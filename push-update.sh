...
updates_dir=/data/clover_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have CloverProject-VERSION-DATE-TYPE-* as format"
...
# Assume CloverProject-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.clover.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.clover.updater/databases/updates.db" \
